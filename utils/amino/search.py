...
from tokens import tokenize, untoken
...
def alg(x: List[str], y: List[str]) -> str:
    ...
    if start == 0:
        raise(RuntimeError)
    z = untoken(x[start[0]], True)
    x = x[start[0]][start[1]:]
    comp = compare(untoken(x, True), untoken(y, True))
    if comp[1] >= 70:
        return z
    else:
        raise RuntimeError
